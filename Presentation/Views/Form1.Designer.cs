namespace Futtage
{
    partial class Form1
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine("=== Form1_Load INICIADO ===");
            System.Diagnostics.Debug.WriteLine($"Presenter definido? {_presenter != null}");
            ...
            // Conectar eventos de corte
            ConnectCuttingEvents();

            // Inicializar estado da UI
            UpdateUIState();

            System.Diagnostics.Debug.WriteLine("=== Form1_Load FINALIZADO ===");
        }
        /// <summary>
        ...
        private void ConnectCuttingEvents()
        {
            System.Diagnostics.Debug.WriteLine("Conectando eventos de corte...");

            // Verificar se os controles existem antes de conectar eventos
            if (txtInicioCorte != null)
            {
                // CORRIGIDO: Usar nome único para evitar conflito com Designer
                txtInicioCorte.TextChanged += OnInicioCorteTextChanged;
                System.Diagnostics.Debug.WriteLine("Evento txtInicioCorte conectado");
            }

            if (txtFimCorte != null)
            {
                // CORRIGIDO: Usar nome único para evitar conflito com Designer
                txtFimCorte.TextChanged += OnFimCorteTextChanged;
                System.Diagnostics.Debug.WriteLine("Evento txtFimCorte conectado");
            }
        }

        private void OnInicioCorteTextChanged(object sender, EventArgs e)
        {
            System.Diagnostics.Debug.WriteLine($"txtInicioCorte mudou para: {txtInicioCorte.Text}");
            UpdateUIState();
        }

        private void OnFimCorteTextChanged(object sender, EventArgs e)
        {
            System.Diagnostics.Debug.WriteLine($"txtFimCorte mudou para: {txtFimCorte.Text}");
            UpdateUIState();
        }

        private bool AreCuttingFieldsValid()
        {
            if (string.IsNullOrWhiteSpace(txtInicioCorte.Text) || string.IsNullOrWhiteSpace(txtFimCorte.Text))
            {
                return false;
            }

            // Verificar se são formatos de tempo válidos
            bool startValid = TimeSpan.TryParse(txtInicioCorte.Text, out var start);
            bool endValid = TimeSpan.TryParse(txtFimCorte.Text, out var end);
            ...
            // Verificar se início é menor que fim
            return start < end;
        }
